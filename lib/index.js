...
const DEFAULTS = {
  ...
  noprompt: isCI || !process.stdout.isTTY,
  access_token: '',
...
(async () => {
  ...
  try {
    ...
    // access_token
    config.access_token = process.env.MINI_APPS_ACCESS_TOKEN || config.access_token || vault.get('access_token');
    ...
    if (e && e.syscall === 'access') {
      ...
      if (message.includes('authorization')) {
        console.error(chalk.red('Access key is invalid.'));
        if (vault) {
          vault.delete('access_token');
        }
      }
      console.error(chalk.red(message));
      console.log('Try to run again or see', link('troubleshooting', 'https://github.com/vkcom/vk-miniapps-deploy#troubleshooting'), 'if problem recurs.');
    }
    process.exit(1);
  }
})();
